...
struct ImGuiPtrOrIndex
{
    ...
    int         m_Index;          // Usually index in a main pool.

    ImGuiPtrOrIndex(void* ptr)  { Ptr = ptr; m_Index = -1; }
    ImGuiPtrOrIndex(int index)  { Ptr = NULL; m_Index = index; }
};
...
{
    ImGuiKeyRoutingIndex            m_Index[ImGuiKey_NamedKey_COUNT]; // Index of first entry in Entries[]
    ImVector<ImGuiKeyRoutingData>   Entries;
    ImVector<ImGuiKeyRoutingData>   EntriesNext;                    // Double-buffer to avoid reallocation (could use a shared buffer)

    ImGuiKeyRoutingTable()          { Clear(); }
    void Clear()                    { for (int n = 0; n < IM_ARRAYSIZE(m_Index); n++) m_Index[n] = -1; Entries.clear(); EntriesNext.clear(); }
};
...
{
    ImGuiTableColumnIdx         m_Index;      // Column index
    ImU32                       TextColor;
    ImU32                       BgColor0;
    ImU32                       BgColor1;
};
